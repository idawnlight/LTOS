...
pub fn init() {
	unsafe {
		...
		let ptr = HEAP_START as *mut PageFlags;
		...
		// (HEAP_START + num_pages * size_of::<PageFlags>() + PAGE_SIZE - 1)
		// & !(PAGE_SIZE - 1);
		ALLOC_START = align_val(
			HEAP_START
				+ num_pages * size_of::<PageFlags,>(),
			PAGE_ORDER,
		);
	}
}
	...
	unsafe {
		...
		for i in 0..num_pages - pages {
			...
			if found {
				...
				// The PageFlags structures themselves aren't the
...
pub fn dealloc(ptr: *mut u8) {
	...
	unsafe {
		...
		let mut p = addr as *mut PageFlags;
		...
		assert_eq!((*p).is_last(), true, "Possible double-free detected! (Not taken found \
		         before last)");
...
pub fn print_page_allocations() {
	unsafe {
		let num_pages = HEAP_SIZE / PAGE_SIZE;
		let mut beg = HEAP_START as *const PageFlags;
